...
[![GoDoc](https://godoc.org/github.com/grill66/fastjson?status.svg)](http://godoc.org/github.com/grill66/fastjson)
[![Go Report](https://goreportcard.com/badge/github.com/grill66/fastjson)](https://goreportcard.com/report/github.com/grill66/fastjson)
...
## Features
  ...
  * Provides simple [API](http://godoc.org/github.com/grill66/fastjson).
  ...
  * May quickly extract a part of the original JSON with `Value.Get(...).MarshalTo` and modify it
    with [Del](https://godoc.org/github.com/grill66/fastjson#Value.Del)
    and [Set](https://godoc.org/github.com/grill66/fastjson#Value.Set) functions.
  * May parse array containing values with distinct types (aka non-homogenous types).
    For instance, `fastjson` easily parses the following JSON array `[123, "foo", [456], {"k": "v"}, null]`.
  * `fastjson` preserves the original order of object items when calling
    [Object.Visit](https://godoc.org/github.com/grill66/fastjson#Object.Visit).


## Known limitations

  * Requies extra care to work with - references to certain objects recursively
    returned by [Parser](https://godoc.org/github.com/grill66/fastjson#Parser)
    must be released before the next call to [Parse](https://godoc.org/github.com/grill66/fastjson#Parser.Parse).
    Otherwise the program may work improperly. The same applies to objects returned by [Arena](https://godoc.org/github.com/grill66/fastjson#Arena).
    Adhere recommendations from [docs](https://godoc.org/github.com/grill66/fastjson).
  * Cannot parse JSON from `io.Reader`. There is [Scanner](https://godoc.org/github.com/grill66/fastjson#Scanner)
...
See also [examples](https://godoc.org/github.com/grill66/fastjson#pkg-examples).
...
## Performance optimization tips
  ...
  * Re-use [Parser](https://godoc.org/github.com/grill66/fastjson#Parser) and [Scanner](https://godoc.org/github.com/grill66/fastjson#Scanner)
    for parsing many JSONs. This reduces memory allocations overhead.
    [ParserPool](https://godoc.org/github.com/grill66/fastjson#ParserPool) may be useful in this case.
  * Prefer calling `Value.Get*` on the value returned from [Parser](https://godoc.org/github.com/grill66/fastjson#Parser)
  ...
  * Prefer calling once [Value.Get](https://godoc.org/github.com/grill66/fastjson#Value.Get)
    for common prefix paths and then calling `Value.Get*` on the returned value
    for distinct suffix paths.
  * Prefer iterating over array returned from [Value.GetArray](https://godoc.org/github.com/grill66/fastjson#Object.Visit)
...
go-fuzz-build github.com/grill66/fastjson
...
```
$ GOMAXPROCS=1 go test github.com/grill66/fastjson -bench='Parse$'
goos: linux
goarch: amd64
pkg: github.com/grill66/fastjson
...
```
$ GOMAXPROCS=1 go test github.com/grill66/fastjson -bench='Validate$'
goos: linux
goarch: amd64
pkg: github.com/grill66/fastjson
...
## FAQ
  ...
  * Q: _There are a ton of other high-perf packages for JSON parsing in Go. Why creating yet another package?_
    A: Because other packages require either rigid JSON schema via struct magic
       ...
       Additionally, `fastjson` provides nicer [API](http://godoc.org/github.com/grill66/fastjson).
  ...
  * Q: _Why fastjson doesn't provide fast marshaling (serialization)?_
    A: Actually it provides some sort of marshaling - see [Value.MarshalTo](https://godoc.org/github.com/grill66/fastjson#Value.MarshalTo).
  ...
  * Q: _`fastjson` crashes my program!_
    A: There is high probability of improper use.
       * Make sure you don't hold references to objects recursively returned by `Parser` / `Scanner`
         ...
         if such restriction is mentioned in [docs](https://github.com/grill66/fastjson/issues/new).
       ...
       * If your program continue crashing after fixing issues mentioned above, [file a bug](https://github.com/grill66/fastjson/issues/new).
